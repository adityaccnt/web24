...
@section('main')
    ...
    <div class="content-wrap ">
        ...
        <div class="position-relative py-210">
            ...
            <div class="container">
                <h1 class="text-white mb-0 text-center">Berita {{ $category->name }}</h1>
            </div>
        </div>
            ...
            <div class="container">
                ...
                <div>
                    <ul class="nav justify-content-center isotope-filters mb-60">
                        <li class="nav-item"><a class="nav-link" href="{{ url('/berita') }}" data-filter="marketing">Semua</a></li>
                        @foreach ($categories as $categories)
                            <li class="nav-item{{ ($category->slug == $categories->slug) ? ' active':''; }}"><a class="nav-link" href="{{ url('berita/kategori/'. $categories->slug) }}">{{ $categories->name }}</a></li>
                        @endforeach
                    </ul>
                        ...
                        <div class=" isotope-item col-12 col-md-6 col-lg-4" data-filters="architecture">
                            ...
                            <div class="card card-blog  card-vertical card-hover-zoom card-blog-bordered rounded-4 overflow-hidden bg-white">
                                ...
                                <div class="card-body py-40 px-50 pb-50"><a class="card-title h4" href="{{ url('berita/' . $post->slug) }}">{{ $post->title }}</a>
                                    <p class="card-text">{!! $post->excerpt !!}</p><!-- Button--><a class="btn btn-accent-1 btn-link btn-clean" href="{{ url('berita/' . $post->slug) }}" target="_self">Selengkapnya</a>
                                </div>
                            </div>
                        </div>
                        @endforeach
